...
export class ProgressManager implements IProgressManager {
  ...
  public updatePhase(
    ...
    const phaseSpeed = this.processedFramesCount
      ? phaseElapsed / this.processedFramesCount
      : 0;
    const remainingInPhase = totalItems
      ? (totalItems - this.processedFramesCount) * phaseSpeed
      : 0;
    ...
    Object.values(GIF_PHASES).forEach((phase) => {
      if (phase.id !== phaseId && !this.completedPhases.has(phase.id)) {
        const isEarlierPhase =
          Object.values(GIF_PHASES).indexOf(phase) <
