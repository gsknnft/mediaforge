...
export class ImageAnalyzer implements IImageAnalyzer {
  ...
  static getInstance(): ImageAnalyzer {
    if (!this.instance) {
      this.instance = new ImageAnalyzer();
    }
    return this.instance;
  }

  destroyInstance() {
    ImageAnalyzer.instance = null;
  }
  ...
  private determineStyle(
    ...
    // Flat icons: very few colors, small, likely SVG-sourced PNG
    if (
      analysis.uniqueColorCount <= 8 &&
      !analysis.hasPartialTransparency &&
      analysis.isHighRes === false
    )
      return "flat-icon";

    // Generated/exported: no transparency, low color density, solid look
    if (
    ...
    // Photo-like: high color count, no transparency
    if (
    ...
    // Illustrated/hand-authored: partial transparency + complex edges
  ...
  private inferOrigin(
    ...
    // Tiny images shipped with npm packages (spinners, placeholders, icons)
    if (bytes < 4096 && analysis.uniqueColorCount <= 16)
      return "dependency-asset";

    // Common generated output naming patterns
    if (
    ...
    // Placeholder / test fixture patterns
    if (/\b(placeholder|fixture|mock|test|spec|sample)\b/.test(lower)) {
      return "test-fixture";
    }

    // Likely source asset: authored name, partial transparency, complex
  ...
  private async decodeImage(buffer: ArrayBuffer): Promise<ImageBitmap> {
    return await createImageBitmap(new Blob([buffer]));
  }

  private getPixelData(bitmap: ImageBitmap): {
    width: number;
    height: number;
