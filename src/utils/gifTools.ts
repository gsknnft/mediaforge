...
import { GIFProgressTracker, GIF_PHASES } from '../GifProcessor/services/GIFProgressTracker';
import { Overlay } from '../assets/services/assetSource';
 ...
 preOptimizeGifFrame(
    ...
    async loadAndCreateStaticImage(
        ...
        try {
          ...
          const x = this.calculateCenteredPosition(this.CONSTANTS.TARGET_SIZE, scaledWidth);
          const y = this.calculateCenteredPosition(this.CONSTANTS.TARGET_SIZE, scaledHeight);
          ...
          this.updatePhase(GIF_PHASES.CREATE_STATIC.id, 100, 'Static layer complete');
          return canvas;
        } catch (error) {
          ...
          public updatePhase(phaseId: string, progress: number, message: string, totalFrames?: number): void {
            ...
            window.dispatchEvent(new CustomEvent('gif-phase-update', {
              detail: {
                ...
                message,
                timestamp: Date.now(),
                eta: totalFrames !== undefined && (phaseId === GIF_PHASES.PROCESSING.id || phaseId === GIF_PHASES.ENCODING.id)
                ? this.calculateETA(Math.floor((progress / 100) * totalFrames), totalFrames)
                : undefined
              }
            }));
    ...
    private calculateETA(currentFrame: number, totalFrames: number): string {
      ...
      optimizeGifFrame(frame: ImageBitmap, enhanceColors: boolean = false): HTMLCanvasElement {
  ...
  calculateGifFitDimensions(frameWidth: number, frameHeight: number): {
    ...
    private normalizeFrameData(data: Uint8Array | Uint8ClampedArray, width: number, height: number): Uint8ClampedArray {
      ...
      console.debug(`Normalizing frame data: ${currentLength} -> ${expectedLength} bytes`);
