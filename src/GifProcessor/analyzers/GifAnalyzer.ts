...
import { ParsedFrame, parseGIF, decompressFrame, decompressFrames } from "gifuct-js";
...
export interface IGifAnalyzer {
    ...
    analyzeGIFFrameDimensions(frames: ParsedFrame[]): {maxWidth: number, maxHeight: number, hasVariableSize: boolean, scaleFactors: number[]};
}
...
export class GifAnalyzer implements IGifAnalyzer {
  ...
  public destroyInstance() {
    ...
    public async detectPixelArtInAllFrames(frames: ParsedFrame[]): Promise<boolean> {
  ...
  public detectPixelArt(frame: ParsedFrame): boolean {
    ...
    public async analyzeGIF(buffer: ArrayBuffer): Promise<GIFMetadata> {
      ...
      const isHighRes = frames.some(frame => 
        frame.dims.width * frame.dims.height > 512 * 512
      );

      let isPixelArt = false;
        isPixelArt = frames.some(frame => 
          this.detectPixelArt(frame)
        );
      ...
      const frameMetrics = frames.map(frame => {
        ...
        for (let i = 0; i < frame.patch.length; i += 4) {
          if (frame.patch[i + 3] > 0) { // Only count non-transparent pixels
            totalUniqueColors.add(`${frame.patch[i]},${frame.patch[i + 1]},${frame.patch[i + 2]}`);
          }
        }
      ...
      const frameExtras = {
        ...
        frameColors: frames.map(f => {
          ...
          for (let i = 0; i < f.patch.length; i += 4) {
            if (f.patch[i + 3] > 0) { // Only include non-transparent pixels
              colors.add((f.patch[i] << 16) | (f.patch[i + 1] << 8) | f.patch[i + 2]);
            }
          }
          return colors;
        }),
        framePixels: frames.map(f => f.pixels || []),
        transparencyThresholds: frameMetrics.map(m => m.needsDisposal ? 220 : 128),
        isHighRes,
    ...
    //public analyzeFrameDimensions(frames: ParsedFrame[]): FrameSizeMetadata {
        public analyzeGIFFrameDimensions(frames: ParsedFrame[]): {
            ...
            const hasVariableSize = dimensions.some(d => 
              d.width !== maxWidth || d.height !== maxHeight
            );
