import { PixelGifScaler } from '../../utils/PixelGifScaler';
import MemoryManager from '../managers/MemoryManager';
import { PixelArtHandler } from '../handlers/PixelArtHandler';
import { ParsedFrame, GIFMetadata, PixelArtMetrics, QualityPresetExtension, ProcessedFrame } from '../types/gif.types';
...
interface IImageProcessingService {
    ...
    calculateUniformDimensions(metadata: GIFMetadata): {width: number; height: number; scale: number};
}
...
class ImageProcessingService implements IImageProcessingService {
    ...
    public static getInstance(workerCount: number, pixelArtHandler: PixelArtHandler, gifAnalyzer: GifAnalyzer): ImageProcessingService {
        if (!this.instance) {
            this.instance = new ImageProcessingService(workerCount, pixelArtHandler, gifAnalyzer);
        }
    ...
    private normalizePatchForImageData(patch: Uint8ClampedArray): Uint8ClampedArray<ArrayBuffer> {
    ...
    public async createImgBitmap(frames: ParsedFrame[], staticImage: HTMLCanvasElement): Promise<ProcessedFrame[]> {
        ...
        if (isPixelArt) {
            ...
            frames = frames.map((frame, i) => this.pixelArtHandler.processPixelArtFrame(frame, frameAnalysis, i));
        }
        ...
        const bufferCtx = bufferCanvas.getContext('2d', { alpha: true, willReadFrequently: true });
        ...
        return Promise.all(frames.map(async (frame, index) => {
            ...
            frameCtx.putImageData(new ImageData(this.normalizePatchForImageData(frame.patch), width, height), 0, 0);
            ...
            if (staticImage) {
                ...
                compositeCtx.drawImage(staticImage, 0, 0, CONSTANTS.TARGET_SIZE, CONSTANTS.TARGET_SIZE);
            }
...
public createCanvas(width: number, height: number): HTMLCanvasElement | OffscreenCanvas {
  ...
  public getCanvasContext(canvas: HTMLCanvasElement | OffscreenCanvas): CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D {
    const ctx = canvas.getContext('2d', { willReadFrequently: true, alpha: true });
    if (!ctx) throw new Error('Failed to get canvas context');
    return ctx;
  }

  private scaleFramePatch(frame: ParsedFrame, ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D): HTMLCanvasElement {
      ...
      const imageData = new ImageData(this.normalizePatchForImageData(frame.patch), frame.dims.width, frame.dims.height);
      tempCtx.putImageData(imageData, 0, 0);
  
      // Draw scaled content onto the main canvas context
      ctx.drawImage(tempCanvas, 0, 0, CONSTANTS.TARGET_SIZE, CONSTANTS.TARGET_SIZE);
  ...
  private overlayStaticImage(ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D, staticImage: HTMLCanvasElement): void {
      ctx.globalCompositeOperation = 'source-over'; // Default composite operation
      ctx.drawImage(staticImage, 0, 0, CONSTANTS.TARGET_SIZE, CONSTANTS.TARGET_SIZE);
  }
...
public async preOptimizeGifFrame(
  ...
  public calculateUniformDimensions(metadata: GIFMetadata): {width: number; height: number; scale: number} {
  ...
  */
  private async optimizePatch(
    ...
    const processedChunks = await Promise.all(
        chunks.map(chunk => this.processChunk(chunk, transparentIndex, settings, metrics))
    );
