...
import { PixelArtSettings, PixelArtMetrics, GifConstants, ParsedFrame } from "../types/gif.types";
...
export class PixelArtHandler {
 private static instance: PixelArtHandler | null = null;
  ...
  public destroyInstance() {
    ...
    public detectPixelArt(frame: ParsedFrame): boolean {
      ...
      for (let i = 0; i < frame.patch.length; i += 4) {
        ...
        uniqueColors.add(`${frame.patch[i]},${frame.patch[i + 1]},${frame.patch[i + 2]}`);
      }
    ...
    private detectSharpEdges(frame: ParsedFrame): boolean {
      ...
      for (let y = 1; y < height - 1; y++) {
        for (let x = 1; x < width - 1; x++) {
          ...
          if (frame.patch[idx + 3] > 0) { // Only check non-transparent pixels
            totalEdges++;
            if (this.isSharpTransition(frame.patch, idx, up) ||
    ...
    private isSharpTransition(patch: Uint8ClampedArray, idx1: number, idx2: number): boolean {
      if (patch[idx2 + 3] === 0) return false; // Skip transparent pixels
      
      const threshold = 32; // Color difference threshold
      return Math.abs(patch[idx1] - patch[idx2]) > threshold ||
  ...
  public processPixelArtFrame(
    frame: ParsedFrame, 
    frameAnalysis: ReturnType<typeof GifAnalyzer.prototype.analyzeGIFFrameDimensions>,
    frameIndex: number
  ): ParsedFrame {
    ...
    if (originalFrameData.left === 0 && originalFrameData.top === 0) {
      position.x = Math.floor((scaledDimensions.targetWidth - scaledDimensions.width) / 2);
      position.y = Math.floor((scaledDimensions.targetHeight - scaledDimensions.height) / 2);
    }
    ...
    outCtx.drawImage(
      tempCanvas,
      0, 0, originalFrameData.width, originalFrameData.height,  // Source rect
      alignedPosition.x, alignedPosition.y,                     // Destination position
      scaledDimensions.width, scaledDimensions.height          // Destination size
    );
  
    // Get the final image data
    const finalData = outCtx.getImageData(0, 0, scaledDimensions.targetWidth, scaledDimensions.targetHeight);
