...
import { GIFMetadata } from "@/types/index_";
  ...
  public async analyzeGif(url: string) {
    try {
      ...
      const recommendedQuality =
        this.qualityManager.selectOptimalQuality(metadata);
...
export const qualityAnalyzerService =
  new QualityAnalyzerService().getInstance();
